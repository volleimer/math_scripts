...
Bausteine sind die Treppenfunktionen $\varphi \in \stair$ mit zugehöriger Unterteilung $a = x_0 < x_1 < \ldots < x_n = b$ und $\varphi = c_k$ auf $(x_{k-1},x_k)$:
...
\begin{rmrk}
 Keine Äquivalenz in (b), zum Beispiel $f: \real \to \real$ gegeben durch $f(x)
 = x $ für alle $x$ mit Maßraum $(\real, \borel(\real), \lebesgue^1 )$.
\end{rmrk}
...
Eine Funktion $f:(0,\infty) \to \real$ heißt uneigentlich Riemann-integrierbar $:\Leftrightarrow$ $\forall 0 < \eps < R < \infty$ gilt $f$ ist Riemann-integrierbar auf $[\eps,R]$. Dann existiert
...
\begin{proof}
 Sei $(t_k)_{k \in \nat} \subset U$ mit $t_k \to a$ und definiere $f_k(x) :=
 f(x, t_k)$ und $f_\infty(x) := f(x, a)$. Nach Voraussetzung ist jedes $f_k$
 messbar und majoriert durch $F$:
 \[ |f_k(x)| = |f(x,t_k)| \le F(x) \text{ für alle } x \in X. \]
 Zudem gilt wegen 2. $f_k \to f$ punktweise auf $X$. Daher folgt die Behauptung
 aus der majorierten Konvergenz. 
\end{proof}
...
\textbf{Formale Anmerkung.} Wenn sich das Volumen einer Menge $E \subset
\real^n$ elemen\-tar\-geo\-metrisch berechnen lässt, dann stimmt es mit dem
Lebesgue-Maß $\lebesgue^n(E)$ überein. Mit anderen Worten: Das $n$-dimensionale
Lebesgue-Maß ist die natürliche Fortsetzung des Volumenbegriffs auf die Klasse
der Borel-messbaren Mengen $\borel(\real^n)$. 
...
\begin{lem}[Cavalieri]
Sei $A \subset \real^{k+\ell}$ eine Borelmenge. Dann gilt
\begin{enumerate}
...
\begin{thm}[Tonelli]{}
 ...
 \[ \int_{\real^\ell} \left( \int_{\real^k} |F(x,y)| \diffop \lebesgue^k (x) ) \right) \diffop \lebesgue^\ell (y) < \infty. \]
...
\begin{proof}
 ...
 Sei $y \in \real^\ell$ fest. Dann ist jedes $\varphi_n(\cdot, y)$ Borel-messbar in $\real^k$, weil der Satz für einfache Funktionen $\varphi_n$ gilt. Da $\varphi_n(\cdot, y) \uparrow f( \cdot, y)$ punktweise auf $\real^k$, folgt mit der monotonen Konvergenz:
